...
const FONT_URLS: Record<string, { regular: string; bold?: string; italic?: string }> = {
  'Georgia': {
    regular: 'https://fonts.gstatic.com/l/font?kit=4UaGrEJMtolZyFvnxnEa0w&skey=a11f9465f7e5e13b&v=v18',
  },
  'Playfair Display': {
    ...
    try {
      Font.register({
        ...
        fonts: [
          ...
          ...(fontData.bold ? [{ src: fontData.bold, fontWeight: 'bold' as const, fontStyle: 'normal' as const }] : []),
          ...(fontData.italic ? [{ src: fontData.italic, fontWeight: 'normal' as const, fontStyle: 'italic' as const }] : []),
        ],
      });
