...
export interface ExplorationParticipant extends ExplorationMarcheur {
  /** When source='participant', `id` is the synthetic key `user:<uuid>` and
   *  `userId` carries the real user id used by the reattribute_media RPC. */
  userId?: string;
  source: 'editorial' | 'participant';
}

// Deterministic emerald-ish color from a name, used when no avatar/couleur set.
 ...
 * credit a photo to anyone who actually walked, even if they don't have an
 * editorial card.
 */
export function useExplorationParticipants(explorationId?: string) {
  return useQuery({
    queryKey: ['exploration-participants', explorationId],
    enabled: !!explorationId,
    staleTime: 60_000,
    queryFn: async (): Promise<ExplorationParticipant[]> => {
      if (!explorationId) return [];

      // 1) Editorial marcheurs (kept verbatim — keep avatars/colors/role/order)
      ...
      // 2) Every validated participant of every marche of this exploration
      ...
      // Editorial first; index editorial user_ids to dedupe
      const editorialUserIds = new Set<string>(
        (editorial || []).map((e: any) => e.user_id).filter(Boolean),
      );

      const editorialMapped: ExplorationParticipant[] = (editorial || []).map((m: any) => {
      ...
      const participantMapped: ExplorationParticipant[] = participantIds
        ...
        .map(uid => {
          ...
          return {
            ...
            speciesObserved: [],
            source: 'participant',
          } satisfies ExplorationParticipant;
        });
