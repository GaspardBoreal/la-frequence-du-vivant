  ...
  return useQuery<any[]>({
    ...
    queryFn: async () => {
      const { data, error } = await db
        .from('iot_capteurs')
        .select('id, nom, serial_number, propriete_id, last_seen_at, actif, silence_alert_hours, etat, maintenance_motif, maintenance_depuis, type:iot_types_capteurs(modele, profondeurs_m, fournisseur_id, fournisseur:iot_fournisseurs(id, nom))')
        .order('nom');
