  ...
  return (
    <>
      ...
      <div className="min-h-screen print-plaquette" style={{ background: 'linear-gradient(180deg, #fefdfb 0%, #f8f6f0 30%, #fefdfb 60%, #f5f3ed 100%)' }}>
        ...
        {/* === HERO === */}
        <section className="relative pt-20 pb-24 md:pt-32 md:pb-36 px-6 text-center overflow-hidden">
          {/* Botanical ornaments */}
          ...
          <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={0} className="relative max-w-3xl mx-auto">
            <motion.span 
              ...
              className="inline-flex items-center gap-2 px-5 py-2 rounded-full text-xs font-semibold tracking-wider uppercase mb-10"
            ...
            <h1 className="font-crimson text-5xl md:text-7xl lg:text-8xl font-semibold leading-[1.02] mb-8" 
              style={{ color: '#1a1a18' }}>
              Devenez<br />
        ...
        <section className="py-20 md:py-28 px-6">
          <div className="max-w-5xl mx-auto">
            <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={0} className="text-center mb-16">
              ...
              <h2 className="font-crimson text-3xl md:text-5xl font-semibold mb-5" style={{ color: '#1a1a18' }}>
            ...
            <div className="grid md:grid-cols-3 gap-5 md:gap-6">
              {piliers.map((p, i) => (
                <motion.div key={p.titre} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i + 1}
                  className={`relative rounded-2xl border ${p.border} ${p.bg} p-7 md:p-8 print-card backdrop-blur-sm overflow-hidden group`}>
                  ...
                  <div className={`w-11 h-11 rounded-xl flex items-center justify-center mb-5 ${p.iconBg} shadow-sm`}>
                    {p.icon}
                  </div>
            ...
            <div className="space-y-6">
              {etapes.map((e, i) => (
                <motion.div key={e.num} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i + 1}
                  className="flex gap-5 md:gap-7 items-start p-5 md:p-7 rounded-2xl transition-colors"
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-5">
              {roles.map((r, i) => (
                <motion.div key={r.nom} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i + 1}
                  className="relative text-center p-6 md:p-7 rounded-2xl print-card group transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5"
                  style={{ background: 'rgba(255,255,255,0.7)', border: '1px solid rgba(214,211,199,0.35)', backdropFilter: 'blur(8px)' }}>
                  <div className={`w-12 h-12 rounded-xl ${r.color} bg-gradient-to-br from-stone-50 to-white flex items-center justify-center mx-auto mb-4 shadow-sm`}
            ...
            <div className="max-w-lg mx-auto space-y-5">
              {zones.map((z, i) => (
                <motion.div key={z.zone} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i + 2}
                  ...
                  <div className="flex-1 h-10 rounded-xl overflow-hidden" style={{ background: 'rgba(214,211,199,0.2)', border: '1px solid rgba(214,211,199,0.2)' }}>
                    <motion.div 
                      initial={{ width: 0 }} 
              ...
              <div className="space-y-8">
                {calendrier.map((c, i) => (
                  <motion.div key={c.date} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i + 1}
                    ...
                    <div className="flex-shrink-0 w-14 h-14 md:w-[4.5rem] md:h-[4.5rem] rounded-2xl flex items-center justify-center z-10 shadow-sm"
                      style={{ 
        ...
        {/* === CTA FINAL === */}
        <section className="py-24 md:py-32 px-6 relative overflow-hidden print:bg-white print:border-t print:border-gray-200">
          <div className="absolute inset-0 print:hidden" style={{
          ...
          <div className="max-w-3xl mx-auto text-center relative">
            <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={0}>
              <div className="w-14 h-14 rounded-2xl mx-auto mb-8 flex items-center justify-center shadow-sm"
                style={{ background: 'linear-gradient(135deg, #ecfdf5 0%, #f0fdfa 100%)', border: '1px solid rgba(16,185,129,0.15)' }}>
                <Sparkles className="w-6 h-6 text-emerald-600" />
              </div>
              
              <h2 className="font-crimson text-3xl md:text-5xl font-semibold mb-6" style={{ color: '#1a1a18' }}>
              ...
              <p className="text-stone-500 mb-10 text-sm tracking-wide">
              ...
              <p className="mt-8 text-xs text-stone-400 font-medium tracking-wide">
