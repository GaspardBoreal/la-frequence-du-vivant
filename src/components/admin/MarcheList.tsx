...
const MarcheList: React.FC<MarcheListProps> = ({ marches, isLoading, onEdit }) => {
  if (isLoading) {
    return (
  ...
  if (marches.length === 0) {
    return (
      <div className="text-center py-12">
        <h3 className="text-lg font-medium text-gray-900 mb-2">Aucune marche trouvée</h3>
        <p className="text-gray-600">Commencez par créer votre première marche</p>
      </div>
    );
  }
          ...
          <div key={marche.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                {marche.theme && (
                  <p className="text-sm text-gray-600 mb-2 font-medium">{marche.theme}</p>
                )}
