...
import { Phone, Copy, Trash2, Pencil, Users, Target } from 'lucide-react';
...
interface Props {
  ...
  counts?: { enroles: number; joints: number; interesses: number; a_appeler: number };
...
export const CampaignCard: React.FC<Props> = ({
  ...
  const taux = joints > 0 ? (interesses / joints) * 100 : 0;
  ...
  const progress = campaign.objectif_contacts
    ? Math.min(100, (joints / campaign.objectif_contacts) * 100)
    : enroles > 0
      ? (joints / enroles) * 100
      : 0;
    ...
    >
      <Card className="group relative overflow-hidden border-[hsl(var(--crm-border))] bg-[hsl(var(--crm-surface))] p-4">
        ...
        <div className="mt-3">
          ...
          <div className="mt-1.5 flex items-center gap-3 text-[11px] crm-muted">
        ...
        <div className="mt-3 flex items-center gap-1.5">
          <Button asChild size="sm" className="h-8 gap-1.5 text-xs">
            <Link to={`/admin/crm/campagnes/${campaign.id}?tab=appels`}>
              ...
              {counts?.a_appeler ? `Appeler (${counts.a_appeler})` : 'Salle d’appels'}
            </Link>
          </Button>
