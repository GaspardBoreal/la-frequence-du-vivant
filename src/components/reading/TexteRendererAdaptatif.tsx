...
export default function TexteRendererAdaptatif({ texte, readingMode = 'rich' }: Props) {
      ...
      <div className="max-w-2xl mx-auto">
        <motion.div
          ...
          className={`
            ...
            ${readingMode !== 'standard' ? 'bg-white/80 dark:bg-slate-800/60 backdrop-blur-sm rounded-3xl shadow-xl border border-slate-300/50 dark:border-slate-700/50' : ''}
          `}
        >
          ...
          >
            ...
            <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold text-slate-900 dark:text-slate-100 leading-relaxed tracking-wide">
              {texte.titre}
            </h1>
            {readingMode === 'rich' && (
          ...
          >
            <div className={`max-w-lg mx-auto ${readingMode === 'rich' ? 'relative' : ''}`}>
              ...
              <div 
                className="text-xl md:text-2xl leading-loose text-slate-900 dark:text-slate-200 font-serif italic font-semibold"
                style={{ 
                  textAlign: 'left',
                  paddingLeft: '3rem',
                  lineHeight: '2.8rem',
          ...
          {readingMode !== 'focus' && (
            ...
            >
              <div className="inline-flex items-center gap-3 px-4 py-2 bg-slate-50 dark:bg-slate-800 rounded-full border border-slate-200 dark:border-slate-700">
                <Badge variant="secondary" className="text-xs font-medium bg-emerald-100 text-emerald-800 dark:bg-emerald-900 dark:text-emerald-300">
                  {texte.marcheName}
                </Badge>
  ...
  const renderDefault = () => (
    <div className={`min-h-screen py-8 px-4 ${
      ...
      >
        <div className={`
          ${readingMode !== 'standard' ? 'bg-white/95 dark:bg-slate-800/80 backdrop-blur-sm border border-slate-300/50 dark:border-slate-700/50 shadow-2xl rounded-3xl' : 'bg-white dark:bg-slate-900 rounded-xl shadow-lg border border-slate-200 dark:border-slate-800'}
            ...
            <div className="text-center mb-8">
              ...
              {readingMode !== 'focus' && (
                <div className="flex items-center justify-center gap-3 text-sm text-slate-600 dark:text-slate-400">
                  <Badge variant="outline" className="bg-slate-50 dark:bg-slate-800 border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300">
