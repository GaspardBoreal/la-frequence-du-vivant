...
import { Camera, Mic, BookOpen, Leaf, Copy, Share2, Users, Sprout } from 'lucide-react';
import { Avatar, AvatarImage, AvatarFallback } from '@/components/ui/avatar';
import { Button } from '@/components/ui/button';
import { useExplorationParticipants, MarcheurWithStats } from '@/hooks/useExplorationParticipants';
...
const MarcheurCard: React.FC<{ marcheur: MarcheurWithStats; index: number }> = ({ marcheur, index }) => {
  ...
  return (
    <motion.div
      ...
      className="flex items-center gap-3 p-3 rounded-xl bg-card border border-border hover:border-emerald-500/30 transition-colors group"
    >
...
const MarcheursTab: React.FC<MarcheursTabProps> = ({ explorationId, marcheEventId, explorationName }) => {
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: 'Marches du Vivant', text, url });
      } catch {
        // User cancelled
      }
    } else {
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2">
        {marcheurs.map((m, i) => (
          <MarcheurCard key={m.id} marcheur={m} index={i} />
        ))}
      </div>
