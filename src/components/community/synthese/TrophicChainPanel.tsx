...
export const TrophicChainPanel: React.FC<Props> = ({ species, explorationId }) => {
  ...
  return (
    <section className={`mt-6 rounded-3xl border border-border bg-gradient-to-br from-card via-card to-muted/20 p-4 sm:p-6 ${open ? 'space-y-5' : ''}`}>
      <header className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <button
          type="button"
          onClick={() => setOpen((o) => !o)}
          className="min-w-0 text-left flex items-start gap-2 group flex-1"
          aria-expanded={open}
        >
          <ChevronDown
            className={`w-4 h-4 mt-1 text-muted-foreground group-hover:text-foreground transition-transform flex-shrink-0 ${open ? '' : '-rotate-90'}`}
          />
