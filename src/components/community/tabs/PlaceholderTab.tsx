import React from 'react';
import { motion } from 'framer-motion';
import { BookOpen, Volume2, Flower2, Globe, Lock } from 'lucide-react';

type PlaceholderType = 'carnet' | 'sons' | 'kigo' | 'territoire';

const META: Record<PlaceholderType, { title: string; desc: string; icon: React.ElementType; accent: string }> = {
  carnet: {
    title: 'Carnet de terrain',
    desc: 'Bientôt, consignez vos observations de biodiversité et partagez vos découvertes avec la communauté.',
    icon: BookOpen,
    accent: 'from-emerald-500/20 to-green-500/10 border-emerald-400/20',
  },
  sons: {
    title: 'Bibliothèque sonore',
    desc: 'Enregistrez les sons du vivant lors de vos marches et contribuez à la bioacoustique participative.',
    icon: Volume2,
    accent: 'from-cyan-500/20 to-blue-500/10 border-cyan-400/20',
  },
  kigo: {
    title: 'Kigo & Haïkus',
    desc: 'Capturez l\'essence des saisons en haïkus. L\'IA du vivant vous accompagne dans la géopoétique japonaise.',
    icon: Flower2,
    accent: 'from-amber-500/20 to-orange-500/10 border-amber-400/20',
  },
  territoire: {
    title: 'Mon territoire',
    desc: 'Visualisez vos explorations sur la carte et découvrez les zones encore inexplorées près de chez vous.',
    icon: Globe,
    accent: 'from-purple-500/20 to-indigo-500/10 border-purple-400/20',
  },
};

interface PlaceholderTabProps {
  type: PlaceholderType;
}

const PlaceholderTab: React.FC<PlaceholderTabProps> = ({ type }) => {
  const { title, desc, icon: Icon, accent } = META[type];

  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      className={`bg-gradient-to-br ${accent} rounded-2xl border p-8 text-center space-y-4`}
    >
      <div className="w-16 h-16 rounded-full bg-white/5 border border-white/10 flex items-center justify-center mx-auto">
        <Icon className="w-8 h-8 text-emerald-200/60" />
      </div>
      <h3 className="text-lg font-semibold text-white">{title}</h3>
      <p className="text-emerald-200/60 text-sm leading-relaxed max-w-sm mx-auto">{desc}</p>
      <div className="inline-flex items-center gap-1.5 bg-white/5 rounded-full px-3 py-1.5 text-xs text-emerald-200/40">
        <Lock className="w-3 h-3" />
        Bientôt disponible
      </div>
    </motion.div>
  );
};

export default PlaceholderTab;
