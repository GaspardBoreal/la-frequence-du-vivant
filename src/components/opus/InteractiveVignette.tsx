...
export const InteractiveVignette: React.FC<InteractiveVignetteProps> = ({
  ...
  return <Card className={`group cursor-pointer transition-all duration-300 hover:shadow-lg hover:scale-105 ${getVariantStyles()} ${className}`}>
      ...
      <div className="space-y-2">
            ...
            <div className="space-y-1">
              {data.nom_scientifique && <p className="text-sm font-normal italic !text-white">
                  {data.nom_scientifique}
                </p>}
              {data.statut_conservation && <p className="text-sm font-bold text-amber-200">
                  {data.statut_conservation}
                </p>}
            </div>
            
            {data.type && <Badge variant="secondary" className="w-fit text-xs mt-2 bg-slate-200 text-slate-900 border-slate-300 font-semibold">
                {data.type}
              </Badge>}
          </div> :
      ...
      <>
            ...
            {data.type && <Badge variant="secondary" className="w-fit text-xs bg-slate-200 text-slate-900 border-slate-300 font-semibold">
                {data.type}
              </Badge>}
          </>}
      </CardHeader>
      ...
      <CardContent className="space-y-3">
        {(data.description_courte || data.definition) && <CardDescription className="text-sm line-clamp-3">
        ...
        <div className="flex items-center justify-between">
            ...
            <DialogContent className="max-w-2xl">
              <DialogHeader>
                <DialogTitle className={`text-xl ${getVariantColor()}`}>
                  {variant === 'species' ? <div className="space-y-2">
                      <div className="text-xl font-bold">
                        {data.nom_commun || data.titre}
                      </div>
                      {data.nom_scientifique && <div className="text-base font-normal italic text-slate-800">
                          {data.nom_scientifique}
                        </div>}
                      {data.statut_conservation && <div className="text-sm font-medium text-emerald-700">
                          {data.statut_conservation}
                        </div>}
                    </div> : data.titre}
                </DialogTitle>
              ...
              <ScrollArea className="max-h-[60vh] pr-4">
                <div className="space-y-4">
                  ...
                  {variant === 'species' && <div className="space-y-4 p-5 bg-emerald-50/80 rounded-xl border border-emerald-200/60">
                      {data.nom_commun && <div>
                  ...
                  {(data.description_courte || data.definition) && <div>
                  ...
                  {data.url && <div className="pt-4 border-t border-border/30">
          ...
          {data.category && <Badge variant="outline" className="text-xs opacity-70">
              {data.category}
            </Badge>}
        </div>
      </CardContent>
