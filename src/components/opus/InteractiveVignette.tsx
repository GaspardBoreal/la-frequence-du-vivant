...
export const InteractiveVignette: React.FC<InteractiveVignetteProps> = ({
  ...
  const sourceIndex = React.useMemo(() => {
    ...
    (importSources || []).forEach((s: any) => {
      const possible = [s?.id, s?.source_id, s?.key, s?.nom, s?.name, s?.url?.split('/')?.pop?.()].filter(Boolean).map((x: any) => String(x).toLowerCase());
  ...
  const resolvedSources = React.useMemo(() => {
    ...
    const normalized = ids.map((x: any) => typeof x === 'string' ? x : x?.id || x?.key || x).filter(Boolean).map((x: any) => String(x).toLowerCase());
    const resolved = normalized.map((id: string) => sourceIndex.get(id)).filter(Boolean);
    return resolved;
  ...
  return <Card className={`group cursor-pointer ${styles.container} ${className}`}>
      ...
      <div className="space-y-2">
            ...
            <div className="space-y-1">
              {data.nom_scientifique && <p className={`text-sm ${styles.secondary}`}>
                  {data.nom_scientifique}
                </p>}
              {data.statut_conservation && <p className={`text-sm ${styles.status}`}>
                  {data.statut_conservation}
                </p>}
            </div>
            
            {data.type && <Badge variant="secondary" className={`w-fit text-xs mt-2 font-semibold ${styles.badge}`}>
                {data.type}
              </Badge>}
            
            {resolvedSources.length > 0 && <Badge variant="outline" className="text-[10px] mt-1">
                {resolvedSources.length} source{resolvedSources.length > 1 ? 's' : ''}
              </Badge>}
          </div> : variant === 'technology' ?
      ...
      <div className="space-y-3">
            ...
            {data.type && <Badge variant="outline" className="w-fit rounded-full border bg-white text-emerald-600 border-emerald-200">
                {data.type}
              </Badge>}
            
            {techMetadata.length > 0 && <div className="flex flex-wrap gap-2 mt-3">
                {techMetadata.slice(0, 2).map((meta, index) => <div key={index} className="flex items-center gap-1.5 text-sm bg-slate-100 text-slate-700 rounded-full px-3 py-1.5 border border-slate-200">
      ...
      <>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {data.type && <Badge variant="secondary" className={`w-fit text-xs font-semibold ${styles.badge}`}>
                  {data.type}
                </Badge>}
              {variant === 'vocabulary' && <Badge variant="outline" className="text-[10px]">
      ...
      <CardContent className="space-y-4">
        {(data.description_courte || data.definition) && <CardDescription className="text-sm line-clamp-3 leading-relaxed font-medium text-gray-200">
        ...
        <div className="flex items-center justify-between">
            ...
            <DialogContent className="max-w-3xl max-h-[90vh] overflow-hidden bg-gradient-to-br from-background via-background/95 to-background/90 backdrop-blur-xl border-border/30">
                  ...
                  {variant === 'vocabulary' ? <div className="flex items-center gap-3 w-full">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          {data.metadata?.resolved_sources?.length && <Badge variant="outline" className="text-xs text-info border-info/30">
                              {data.metadata.resolved_sources.length} source{data.metadata.resolved_sources.length > 1 ? 's' : ''}
                            </Badge>}
                        </div>
                      </div>
                    </div> : variant === 'species' ? <div className="space-y-2">
                      <div className="text-xl font-bold">
                        {data.nom_commun || data.titre}
                      </div>
                      {data.nom_scientifique && <div className="text-base font-normal italic text-slate-600">
                          {data.nom_scientifique}
                        </div>}
                      {data.statut_conservation && <div className="text-sm font-medium text-muted-foreground">
                          {data.statut_conservation}
                        </div>}
                    </div> : variant === 'technology' ? <div className="flex items-center gap-3 w-full">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-2 mt-1 flex-wrap">
                          {data.type && <Badge variant="secondary" className={`text-xs font-semibold ${getTechTypeBadgeColor(data.type)}`}>
                              {data.type}
                            </Badge>}
                          {data.category && <Badge variant="outline" className="text-xs">
                              {data.category}
                            </Badge>}
                        </div>
                      </div>
                      ...
                      {data.type && <Badge variant="secondary" className="text-xs">
                          {data.type}
                        </Badge>}
                    </div>}
                </DialogTitle>
              ...
              <ScrollArea className="flex-1 pr-4">
                <div className="space-y-6 py-4">
                  ...
                  {variant === 'technology' && <div className="space-y-4">
                      {/* Caractéristiques techniques */}
                      {techMetadata.length > 0 && <div className="p-6 rounded-xl bg-gradient-to-br from-primary/5 to-primary/5 border border-primary/20">
                          ...
                          <div className="grid gap-3">
                            {techMetadata.map((meta, index) => <div key={index} className="flex items-center gap-3 p-3 rounded-lg bg-background/50 border border-border/30">
                      ...
                      {data.metadata?.impact_territorial && <div className="p-4 rounded-lg bg-success/5 border border-success/10">
                      ...
                      {data.metadata?.liens && Array.isArray(data.metadata.liens) && data.metadata.liens.length > 0 && <div className="p-6 rounded-xl bg-gradient-to-br from-info/5 to-info/5 border border-info/20">
                          <div className="flex items-center gap-2 mb-4">
                          ...
                          <div className="grid gap-3">
                            {data.metadata.liens.map((lien: string, index: number) => <div key={index} className="flex items-center gap-3 p-3 rounded-lg bg-background/50 border border-border/30 hover:bg-background/80 transition-colors">
                                ...
                                <div className="min-w-0 flex-1">
                                  {lien.startsWith('http') ? <a href={lien} target="_blank" rel="noopener noreferrer" className="text-sm text-info hover:text-info/80 underline truncate block" title={lien}>
                                      {lien.length > 60 ? `${lien.substring(0, 60)}...` : lien}
                                    </a> : <p className="text-sm text-muted-foreground">
                                      {lien}
                                    </p>}
                                </div>
                                {lien.startsWith('http') && <Button variant="ghost" size="sm" onClick={() => window.open(lien, '_blank')} className="w-8 h-8 p-0 text-info hover:text-info hover:bg-info/10">
                  ...
                  {variant === 'vocabulary' && data.metadata && <div className="grid gap-4">
                      {(data.metadata.origine || data.metadata.etymologie) && <div className="p-4 rounded-lg bg-info/5 border border-info/10">
                          <div className="flex items-center gap-2 mb-2">
                      ...
                      {data.metadata.usage_context && <div className="p-4 rounded-lg bg-accent/5 border border-accent/10">
                          <div className="flex items-center gap-2 mb-2">
                  ...
                  {variant === 'species' && <div className="space-y-4">
                      ...
                      {resolvedSources.length > 0 && <div className="p-6 rounded-xl bg-gradient-to-br from-success/5 to-success/5 border border-success/20">
                          <div className="flex items-center gap-2 mb-4">
                        ...
                        const shortName = href && /^https?:\/\//i.test(href) ? new URL(href).hostname.replace('www.', '').split('.')[0] : 'Source';
                        const displayTitle = title || shortName || `Source ${index + 1}`;
                        const date = source?.date_acces;
                        return <div key={source?.id || index} className="flex items-center gap-3 p-3 rounded-lg bg-background/50 border border-border/30 hover:bg-background/80 transition-colors">
                                  ...
                                  <div className="min-w-0 flex-1">
                                    ...
                                    {href && /^https?:\/\//i.test(href) ? <a href={href} target="_blank" rel="noopener noreferrer" className="text-xs text-success hover:text-success/80 underline truncate block mt-1" title={href}>
                                        {href.length > 50 ? `${href.substring(0, 50)}...` : href}
                                      </a> : <p className="text-xs text-muted-foreground mt-1">
                                        URL non disponible
                                      </p>}
                                  </div>
                                  {href && /^https?:\/\//i.test(href) && <Button variant="ghost" size="sm" onClick={() => window.open(href, '_blank')} className="w-8 h-8 p-0 text-success hover:text-success hover:bg-success/10">
                                      <ExternalLink className="w-4 h-4" />
                                    </Button>}
                                </div>;
                      })}
                  ...
                  {data.details && <div className="p-4 rounded-lg bg-muted/20 border border-border/20">
                  ...
                  {Array.isArray(data.metadata?.resolved_sources) && data.metadata.resolved_sources.length > 0 && <div className="p-6 rounded-xl bg-gradient-to-br from-info/5 to-info/5 border border-info/20">
                      <div className="flex items-center gap-2 mb-4">
                        <ExternalLink className="w-5 h-5 text-info" />
                        <h3 className="font-semibold text-base text-info">
                      ...
                      return <div key={source?.id || index} className="flex items-center gap-3 p-3 rounded-lg bg-background/50 border border-border/30 hover:bg-background/80 transition-colors">
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                {href && /^https?:\/\//i.test(href) ? <a href={href} target="_blank" rel="noopener noreferrer" className="text-xs text-info hover:text-info/80 underline truncate block mt-1" title={href}>
                                    {href.length > 50 ? `${href.substring(0, 50)}...` : href}
                                  </a> : <p className="text-xs text-muted-foreground mt-1">
                                    URL non disponible
                                  </p>}
                              </div>
                              {href && /^https?:\/\//i.test(href) && <Button variant="ghost" size="sm" onClick={() => window.open(href, '_blank')} className="w-8 h-8 p-0 text-info hover:text-info hover:bg-info/10">
                                  <ExternalLink className="w-4 h-4" />
                                </Button>}
                            </div>;
                    })}
          ...
          {data.url && /^https?:\/\//i.test(data.url) && <Button variant="ghost" size="sm" onClick={() => window.open(data.url, '_blank')} className="text-xs hover:bg-background/50">
