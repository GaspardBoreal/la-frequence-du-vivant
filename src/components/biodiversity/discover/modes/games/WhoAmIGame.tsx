...
const WhoAmIGame: React.FC<Props> = ({ species, photoBy }) => {
  ...
  const { target, options } = useMemo(() => {
    ...
    const t = picks[0];
    return { target: t, options: shuffle(picks) };
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
        <div className="relative aspect-square rounded-3xl overflow-hidden border-2 border-[#3B2A1A]/20 bg-white shadow-[6px_6px_0_rgba(59,42,26,0.15)]">
          ...
          />
          <div className="absolute top-3 left-3 px-3 py-1 rounded-full bg-amber-200/90 text-amber-900" style={{ fontFamily: '"Caveat", cursive', fontSize: 22 }}>
            Qui suis-je ?
          </div>
        </div>
