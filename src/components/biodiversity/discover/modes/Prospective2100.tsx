...
const ProspectiveDetail: React.FC<{ card: Card; onClose: () => void }> = ({ card, onClose }) => {
  ...
  const askIA = async () => {
    ...
    try {
      ...
      const text = (data as { narrative?: string })?.narrative;
      setAiNarrative(text || null);
    } catch (e) {
      setAiError("L'IA n'a pas pu répondre. Réessayez plus tard.");
    } finally {
      setAiLoading(false);
    }
  };
    ...
    >
      ...
      >
        ...
        <div className="p-5 sm:p-6">
          ...
          {aiError && <p className="mt-3 text-xs text-rose-300">{aiError}</p>}
          ...
          >
            ...
            {aiLoading ? 'Génération…' : 'Approfondir avec l\'IA'}
          </button>
        </div>
      </motion.div>
    </motion.div>
