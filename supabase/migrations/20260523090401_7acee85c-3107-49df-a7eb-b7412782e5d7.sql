...
-- Storage bucket for Pack Vivant ZIPs (private — access via signed URLs)
INSERT INTO storage.buckets (id, name, public)
VALUES ('pack-vivant', 'pack-vivant', false)
ON CONFLICT (id) DO NOTHING;

-- Only service role writes; authenticated users cannot directly access objects
CREATE POLICY "Service role manages pack-vivant"
ON storage.objects
FOR ALL
TO service_role
USING (bucket_id = 'pack-vivant')
WITH CHECK (bucket_id = 'pack-vivant');

-- RPC: returns all data needed to build the Pack Vivant for an exploration
...
DECLARE
  ...
  v_anonymize boolean;
BEGIN
  -- Anonymize observers for public level
  ...
  -- Collect all marche_ids for this exploration
  ...
  -- Observations (row per sighting) — UNION of marcheur observations and iNat snapshot species
  WITH marcheur_obs AS (
    SELECT
      ...
      CASE
        ...
        ELSE COALESCE(p.first_name || ' ' || p.last_name, 'Marcheur·euse')
    ...
    LEFT JOIN profiles p ON p.user_id = mo.marcheur_id
  ...
  -- Species synthesis (deduplicated by scientific name, 1 row per species)
  WITH all_obs AS (
    SELECT
      lower(trim(mo.species_scientific_name)) AS sci_key,
      mo.species_scientific_name AS scientific_name,
...
-- Grant execute to authenticated users (level filtering happens server-side)
REVOKE ALL ON FUNCTION public.get_exploration_export_data(uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_exploration_export_data(uuid, text) TO authenticated, anon, service_role;
