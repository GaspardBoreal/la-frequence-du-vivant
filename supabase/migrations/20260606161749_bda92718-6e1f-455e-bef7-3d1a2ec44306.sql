-- Patch route 'text' : send to Synthèse > Textes écrits sub-tab
...
BEGIN
  ...
  species_with_exp AS (
    ...
    LEFT JOIN LATERAL (
      SELECT exploration_id FROM exploration_marches WHERE marche_id = sm.last_marche_id LIMIT 1
    ) em ON true
  ),
  species_contexts AS (
  ...
  SELECT * FROM (
    ...
    LEFT JOIN LATERAL (
      SELECT exploration_id FROM exploration_marches WHERE marche_id = t.marche_id LIMIT 1
    ) em ON true
