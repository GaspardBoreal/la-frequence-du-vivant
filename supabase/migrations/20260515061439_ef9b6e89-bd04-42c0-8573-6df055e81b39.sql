-- Recherche admin de marcheurs pouvant être ajoutés comme Lecteur sur un événement
...
BEGIN
  ...
  AND NOT EXISTS (
    ...
    JOIN public.marches m ON m.id = mp.marche_id
    WHERE m.event_id = _event_id
      AND mp.user_id = cp.user_id
      AND mp.statut = 'validee'
  )
...
-- Ajout direct d'un marcheur existant comme Lecteur invité
...
BEGIN
  ...
  WHERE user_id = _user_id
    ...
    AND NOT EXISTS (
      SELECT 1 FROM public.marche_participations mp
      WHERE mp.user_id = _user_id AND mp.statut = 'validee'
    );
