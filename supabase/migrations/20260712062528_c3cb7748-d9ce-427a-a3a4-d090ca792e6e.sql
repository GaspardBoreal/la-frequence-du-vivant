...
BEGIN
  ...
  -- Espèces snapshot passant le filtre rayon (même logique que get_exploration_species_count)
  snap_species AS (
    SELECT
      sr.marche_id,
  ...
  -- Attributions individuelles (pour compter les observations + dates)
  ...
  snap_obs_dedup AS (
    -- Dédup géographique : même observateur + même date + même lat/lon arrondi = 1 obs
    SELECT key,
  ...
  -- Fusion pour métadonnées (nom, kingdom, rank)
  meta AS (
    SELECT
      key,
