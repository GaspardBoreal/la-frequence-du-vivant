...
BEGIN
  ...
  , species_matches AS (
    SELECT
      ...
      MAX(me.exploration_id::text) AS any_exploration_id,
      GREATEST(
  ...
  , species_contexts AS (
    ...
    LEFT JOIN LATERAL (
      ...
      LEFT JOIN marche_events me2 ON me2.id = o2.marche_id
    ...
    SELECT
      ...
      jsonb_build_object(
        ...
        'excerpt', (
          ...
          END
          FROM (
            SELECT regexp_replace(COALESCE(t.contenu,''), '<[^>]+>', '', 'g') AS plain
          ) p1
          CROSS JOIN LATERAL (
            SELECT POSITION((SELECT s FROM q) IN public.f_unaccent(lower(plain))) AS pos
          ) p2
        )
      )
