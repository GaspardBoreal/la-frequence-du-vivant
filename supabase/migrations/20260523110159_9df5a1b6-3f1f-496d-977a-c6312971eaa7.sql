...
BEGIN
  ...
  SELECT to_jsonb(e.*) INTO v_exploration FROM explorations e WHERE e.id = p_exploration_id;
  IF v_exploration IS NULL THEN
    RETURN jsonb_build_object('error', 'exploration_not_found');
  END IF;

  SELECT array_agg(marche_id) INTO v_marche_ids FROM exploration_marches WHERE exploration_id = p_exploration_id;
  ...
  WITH marcheur_obs AS (
    SELECT
      ...
      st.common_name_fr AS common_name_fr,
    ...
    LEFT JOIN species_translations st
      ON lower(trim(st.scientific_name)) = lower(trim(mo.species_scientific_name))
  ...
  snapshot_obs AS (
    SELECT
      ...
      COALESCE(st.common_name_fr, NULLIF(trim(COALESCE(sp->>'commonName', sp->>'common_name')), '')) AS common_name_fr,
    ...
    LEFT JOIN species_translations st
      ON lower(trim(st.scientific_name)) = lower(trim(COALESCE(sp->>'scientificName', sp->>'scientific_name')))
  ...
  WITH all_obs2 AS (
    ...
    SELECT
      lower(trim(COALESCE(sp->>'scientificName', sp->>'scientific_name'))) AS sci_key,
      COALESCE(sp->>'scientificName', sp->>'scientific_name') AS scientific_name,
      NULLIF(trim(COALESCE(sp->>'commonName', sp->>'common_name')), '') AS common_name_raw,
  ...
  FROM (
    SELECT
      a.sci_key,
      MIN(a.scientific_name) AS scientific_name,
      COALESCE(MAX(st.common_name_fr), MAX(a.common_name_raw)) AS common_name_fr,
      COUNT(*) AS observation_count,
      COUNT(DISTINCT a.observer) AS observer_count,
      MIN(a.observation_date)::text AS first_observation,
      MAX(a.observation_date)::text AS last_observation,
      AVG(a.latitude) FILTER (WHERE a.latitude IS NOT NULL) AS centroid_lat,
      AVG(a.longitude) FILTER (WHERE a.longitude IS NOT NULL) AS centroid_lng,
      array_agg(DISTINCT a.source) AS sources
    FROM all_obs2 a
    LEFT JOIN species_translations st
      ON lower(trim(st.scientific_name)) = a.sci_key
    WHERE a.sci_key IS NOT NULL AND a.sci_key <> ''
    GROUP BY a.sci_key
