...
BEGIN
  ...
  -- Resolve event scope (marche_event.id → exploration_id)
  ...
  -- Set of marche_ids (places) in scope when filtering by event
  scope_marches AS (
    SELECT marche_id FROM exploration_marches WHERE v_exploration_id IS NOT NULL AND exploration_id = v_exploration_id
  ),

  -- ============= ESPÈCES (fusionnées par scientific_name) =============
  species_matches AS (
    SELECT
      ...
      COUNT(DISTINCT o.marche_id) AS marches_count,
      COUNT(DISTINCT o.marcheur_id) AS marcheurs_count,
      MAX(o.observation_date) AS last_observation_date,
      (
        SELECT em.exploration_id::text
        FROM exploration_marches em
        WHERE em.marche_id = MAX(o.marche_id)
        LIMIT 1
  ...
  SELECT * FROM (
    SELECT
      ...
      CASE
        WHEN sm.marches_count > 1 THEN ('Vue '||sm.occurrences||'× sur '||sm.marches_count||' marches')
        ELSE NULL
      END::text AS context,
      sm.score,
      ...
      jsonb_build_object(
        ...
        'excerpt', (
          SELECT CASE
            ...
            ELSE LEFT(COALESCE(tt.quote,''), 120)
          END
