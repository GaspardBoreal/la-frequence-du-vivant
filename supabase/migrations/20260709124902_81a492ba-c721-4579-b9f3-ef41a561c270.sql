...
BEGIN
  ...
  -- Contributions (medias, observations, textes, audio)
  contribs AS (
    SELECT u.user_id,
  ...
  -- Persona members (for export / CTAs)
  persona_members AS (
    SELECT persona,
  ...
  -- Bubble scatter (all users)
  bubble AS (
    SELECT jsonb_agg(jsonb_build_object(
      'user_id', user_id,
      'name', COALESCE(NULLIF(TRIM(COALESCE(prenom,'') || ' ' || COALESCE(nom,'')), ''), 'Marcheur·euse'),
      'x', GREATEST(0, 90 - LEAST(days_since_active, 90)), -- recency (0..90)
      'y', events_30d,
  ...
  -- Feature adoption radar (per persona × event_target)
  radar_raw AS (
    SELECT up.persona,
      CASE
