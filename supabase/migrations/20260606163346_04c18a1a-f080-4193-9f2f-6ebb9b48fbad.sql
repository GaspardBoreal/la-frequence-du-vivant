-- 1) Fix bug: qualify table column to avoid clash with OUT param `id`
...
BEGIN
  ...
  species_matches AS (
    SELECT
      ...
      MAX(o.thumbnail_url) AS thumb_url,
  ...
  SELECT * FROM (
    ...
    SELECT
      ...
      ('/marches-du-vivant/mon-espace/exploration/'||c.exploration_id::text
        ||'?focus=practice:'||c.id::text||'&tab=apprendre')::text,
      jsonb_build_object(
  ...
  SELECT * FROM (
    SELECT
      ...
      (CASE WHEN em.exploration_id IS NOT NULL
        THEN '/marches-du-vivant/mon-espace/exploration/'||em.exploration_id::text
             ||'?focus=text:'||t.id::text
             ||'&tab=biodiversite&sub=textes'
        ELSE '/marche/'||t.marche_id::text
       END)::text,
      jsonb_build_object(
  ...
  SELECT * FROM (
    SELECT
      ...
      ('/marches-du-vivant/mon-espace/exploration/'||COALESCE(me.exploration_id::text,'')
        ||'?focus=testimony:'||tt.id::text||'&tab=biodiversite&sub=temoignages')::text,
      jsonb_build_object(
  ...
  SELECT * FROM (
    SELECT
      ...
      ('/marches-du-vivant/mon-espace/exploration/'||COALESCE(me.exploration_id::text, me.id::text)
        ||'?focus=event:'||me.id::text)::text,
      jsonb_build_object(
...
-- 2) Ensure trigram indexes exist on hot search columns (idempotent)
CREATE INDEX IF NOT EXISTS idx_mo_sci_name_trgm
  ON public.marcheur_observations USING gin (public.f_unaccent(lower(species_scientific_name)) extensions.gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_mo_common_fr_trgm
  ON public.marcheur_observations USING gin (public.f_unaccent(lower(taxon_common_name_fr)) extensions.gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_marche_textes_titre_trgm
  ON public.marche_textes USING gin (public.f_unaccent(lower(titre)) extensions.gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_event_testimonies_quote_trgm
  ON public.event_testimonies USING gin (public.f_unaccent(lower(quote)) extensions.gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_community_profiles_name_trgm
  ON public.community_profiles USING gin (public.f_unaccent(lower(coalesce(prenom,'')||' '||coalesce(nom,''))) extensions.gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_marche_events_title_trgm
  ON public.marche_events USING gin (public.f_unaccent(lower(title)) extensions.gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_exploration_curations_title_trgm
  ON public.exploration_curations USING gin (public.f_unaccent(lower(title)) extensions.gin_trgm_ops);