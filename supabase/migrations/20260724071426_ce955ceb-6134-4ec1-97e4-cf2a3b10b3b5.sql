...
BEGIN
  ...
  -- Dernière date d'observation "vraie" :
  -- 1) marcheur_observations.observation_date
  -- 2) attributions[].observedOn dans les snapshots (dans le rayon)
  -- 3) sp->>'observedOn' | 'lastObservedAt' | 'observed_on'
  ...
  last_snap_att_obs AS (
    SELECT max(NULLIF(att->>'observedOn','')::timestamptz) AS d
    FROM snap_in_radius sr,
    LATERAL jsonb_array_elements(
      CASE WHEN jsonb_typeof(sr.sp->'attributions') = 'array' THEN sr.sp->'attributions' ELSE '[]'::jsonb END
    ) AS att
    WHERE (att->>'observedOn') IS NOT NULL
  )
