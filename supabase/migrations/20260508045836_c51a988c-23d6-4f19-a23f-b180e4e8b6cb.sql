-- 1) Add attributed_user_id to marcheur_textes
ALTER TABLE public.marcheur_textes
  ADD COLUMN IF NOT EXISTS attributed_user_id uuid REFERENCES auth.users(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_marcheur_textes_attributed_user
  ON public.marcheur_textes(attributed_user_id)
  WHERE attributed_user_id IS NOT NULL;

-- 2) Allow 'texte' in attribution log source check (if a CHECK constraint exists)
DO $$
DECLARE
  con RECORD;
BEGIN
  FOR con IN
    ...
    WHERE conrelid = 'public.media_attribution_log'::regclass
      AND contype = 'c'
  LOOP
    EXECUTE format('ALTER TABLE public.media_attribution_log DROP CONSTRAINT %I', con.conname);
  END LOOP;
END $$;

ALTER TABLE public.media_attribution_log
  ADD CONSTRAINT media_attribution_log_source_check
  CHECK (source IN ('media', 'audio', 'conv', 'texte'));

-- 3) Updated reattribute_media supporting 'texte'
...
BEGIN
  ...
  ELSE -- texte
    SELECT mt.attributed_user_id, e.id
      INTO _previous_user, _exploration_id
  ...
  IF _marcheur_id IS NULL AND _user_id IS NOT NULL THEN
    ...
    -- For non-text sources we still resolve a marcheur row; for text we just keep _resolved_user
    IF _source <> 'texte' THEN
  ...
  ELSIF _marcheur_id IS NOT NULL THEN
    ...
    -- For text reattribution to an editorial card, we need an auth user_id
    IF _source = 'texte' AND _resolved_user IS NULL THEN
      RAISE EXCEPTION 'Cannot reattribute text to an editorial marcheur with no linked account' USING ERRCODE = '22023';
    END IF;
  END IF;
  ...
  ELSE -- texte
    UPDATE public.marcheur_textes
       SET attributed_user_id = _resolved_user, updated_at = now()
     WHERE id = _media_id;
  END IF;
