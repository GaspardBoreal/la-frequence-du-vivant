...
CREATE OR REPLACE FUNCTION public.get_community_usage_dashboard(
...
BEGIN
  ...
  IF NOT public.check_is_admin_user(v_uid) THEN
  ...
  kpis AS (
    SELECT
      ...
      COALESCE(ROUND(AVG(events_30d)::numeric, 1), 0) AS avg_events_30d,
      COALESCE(ROUND(AVG(contrib_count)::numeric, 1), 0) AS avg_contribs
    FROM users_persona
  ),
  persona_counts AS (
  ...
  SELECT jsonb_build_object(
    'kpis',       COALESCE((SELECT to_jsonb(kpis) FROM kpis), '{"total_users":0,"dau_7d":0,"wau_30d":0,"mau_90d":0,"with_participation":0,"returning":0,"contributors":0,"adherents":0,"avg_events_30d":0,"avg_contribs":0}'::jsonb),
    'personas',   COALESCE((SELECT jsonb_agg(jsonb_build_object('key', persona, 'count', count)) FROM persona_counts), '[]'::jsonb),
    'persona_members', COALESCE((SELECT jsonb_object_agg(persona, members) FROM persona_members), '{}'::jsonb),
    'bubble',     COALESCE((SELECT points FROM bubble), '[]'::jsonb),
    'heatmap',    COALESCE((SELECT grid FROM heatmap), '[]'::jsonb),
    'radar',      COALESCE((SELECT data FROM radar), '[]'::jsonb),
    'funnel',     COALESCE((SELECT f FROM funnel), '{"inscrits":0,"actifs_30d":0,"participants":0,"fideles":0,"contributeurs":0,"adherents":0}'::jsonb),
    'top_cities', COALESCE((SELECT cities FROM top_cities), '[]'::jsonb),
