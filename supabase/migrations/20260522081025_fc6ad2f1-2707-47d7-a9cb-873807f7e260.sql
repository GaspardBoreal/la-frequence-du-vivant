...
ALTER TABLE public.marche_participations
  ADD COLUMN IF NOT EXISTS is_guest boolean NOT NULL DEFAULT false;

-- 2) get_public_event_stats : source de vérité unique pour les 6 indicateurs
...
BEGIN
  ...
  -- Découvreurs (vues anonymes, déduplication par session)
  ...
  -- Marcheurs : union exploration_marcheurs (non-guest) + participations (non-guest)
  WITH walkers AS (
    SELECT em.user_id
    FROM public.exploration_marcheurs em
    WHERE em.exploration_id = _exploration_id
      AND em.is_guest = false
  ...
  IF _exploration_id IS NOT NULL THEN
    -- Espèces : dernier snapshot par marche + observations marcheurs, dédup par scientific_name
    WITH last_snaps AS (
    ...
    snap_species AS (
      SELECT
        lower((s->>'scientific_name')::text) AS k,
        COALESCE((s->>'observations_count')::int, 1) AS obs
      FROM last_snaps ls
    ...
    merged AS (
      SELECT k, obs FROM snap_species
      UNION ALL
      SELECT k, obs FROM walker_species
    )
    SELECT
      count(DISTINCT k),
      COALESCE(sum(obs), 0)
    INTO _species, _observations
    FROM merged;

    -- Pratiques emblématiques : curation_marcheurs liées à l'exploration
    ...
    -- Échantillon pratiques pour popup (limit 8)
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      'id', ec.id,
      'titre', ec.title,
      'category', ec.category,
      'description', left(COALESCE(ec.description,''), 200),
      'prenom', em.prenom,
      'nom', em.nom,
      'avatar_url', em.avatar_url,
      'photo_url', (
        SELECT mm.url_fichier
    ...
    -- Paysages sonores : marcheur_audio sur les marches de l'exploration
    SELECT count(*) INTO _paysages
    FROM public.marcheur_audio ma
    JOIN public.exploration_marches em ON em.marche_id = ma.marche_id
    WHERE COALESCE(ma.is_public, true) = true;

    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      'id', ma.id,
      'titre', COALESCE(ma.titre, 'Paysage sonore'),
      'description', ma.description,
      'url', ma.url_fichier,
      'duree_secondes', ma.duree_secondes,
      'prenom', em2.prenom,
      'nom', em2.nom,
      'avatar_url', em2.avatar_url
  ...
  RETURN jsonb_build_object(
    'marcheurs_count', _marcheurs,
    'species_count', _species,
    'observations_count', _observations,
    'views_total', COALESCE(_views, 0),
    'unique_visitors', COALESCE(_unique, 0),
    'pratiques_count', _pratiques,
    'paysages_sonores_count', _paysages,
    'pratiques_sample', _pratiques_sample,
    'paysages_sample', _paysages_sample,
    'methodology', jsonb_build_object(
      ...
      'species', 'Dernier snapshot biodiversité par marche de l''exploration + marcheur_observations non liées à iNaturalist, déduplication par nom scientifique.',
      'observations', 'Somme des observations_count des snapshots + observations marcheurs sans doublon iNaturalist.',
      'discoverers', 'Nombre de sessions navigateur uniques ayant ouvert la page publique (anonyme).',
      'pratiques', 'Curations de pratiques emblématiques rattachées aux marcheurs de cette exploration.',
      'paysages', 'Paysages sonores enregistrés par les marcheurs sur les marches de l''exploration.'
    )
  );
END;
$function$;

-- 3) get_public_event_biodiversity : ajout du marche_ordre
...
DECLARE
  _event_id uuid;
  _exploration_id uuid;
  _species jsonb := '[]'::jsonb;
  _observations jsonb := '[]'::jsonb;
  _trophic jsonb;
  _bio_index numeric;
  _snap_date date;
BEGIN
  ...
  IF _exploration_id IS NOT NULL THEN
    -- Espèces issues des derniers snapshots des marches de l'exploration, avec marche_ordre
    WITH last_snaps AS (
    ...
    walker_species AS (
      SELECT
        lower(mo.species_scientific_name) AS k,
    ...
    deduped AS (
      SELECT
        ...
        min(marche_ordre) AS marche_ordre,
    ...
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'observation_date', mo.observation_date,
      'marche_ordre', xm.ordre
    ...
    -- Index moyen + date snapshot la plus récente
    SELECT avg(biodiversity_index), max(snapshot_date)
    INTO _bio_index, _snap_date
    FROM (
