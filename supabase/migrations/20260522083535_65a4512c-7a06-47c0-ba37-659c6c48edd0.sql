...
BEGIN
  ...
  IF _type = 'marche_event' THEN
    ...
    SELECT me.exploration_id INTO _expl_id FROM marche_events me WHERE me.id = _uuid;
    ...
    SELECT jsonb_build_object(
      ...
      'marches', (
        SELECT jsonb_agg(jsonb_build_object(
          ...
          'lat', round(m.latitude::numeric, 5),
          'lon', round(m.longitude::numeric, 5),
          'distance_km', m.distance_km,
          'theme', m.theme_principal,
          'etapes_count', (SELECT count(*) FROM exploration_waypoints w WHERE w.after_marche_id = m.id)
        ) ORDER BY em.ordre)
      ...
      'waypoints', (
        SELECT jsonb_agg(jsonb_build_object(
          'marche_ordre', em.ordre,
          'etape', w.ordre,
          'lat', round(w.latitude::numeric, 5),
          'lon', round(w.longitude::numeric, 5),
          'label', w.label
        ) ORDER BY em.ordre, w.ordre)
        FROM exploration_waypoints w
        JOIN exploration_marches em ON em.marche_id = w.after_marche_id
        WHERE em.exploration_id = me.exploration_id
      ),
