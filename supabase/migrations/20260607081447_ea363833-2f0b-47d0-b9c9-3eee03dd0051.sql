...
BEGIN
  ...
  -- Noms scientifiques candidats résolus depuis les traductions (vernaculaire/alias FR)
  matched_sci AS (
    SELECT DISTINCT st.scientific_name
    FROM public.species_translations st
    WHERE public.f_unaccent(lower(coalesce(st.common_name_fr,''))) ILIKE '%'||(SELECT s FROM q)||'%'
  ...
  unified_obs AS (
    SELECT
      ...
      o.inaturalist_observation_id::text AS inat_id,
    ...
    WHERE (v_exploration_id IS NULL OR bs.marche_id IN (SELECT marche_id FROM scope_marches))
      AND (
        -- match direct sur sciName ou commonName
        public.f_unaccent(lower((sp->>'scientificName')::text)) ILIKE '%'||(SELECT s FROM q)||'%'
        OR public.f_unaccent(lower(coalesce((sp->>'commonName')::text,''))) ILIKE '%'||(SELECT s FROM q)||'%'
        -- ou résolu via species_translations (ex: "Buddleja de David" -> "Buddleja davidii")
        OR (sp->>'scientificName')::text IN (SELECT scientific_name FROM matched_sci)
      )
  ),
  unified_dedup AS (
    SELECT u.*
  ...
  species_matches AS (
    ...
    WHERE
        ...
        OR u.scientific_name IN (SELECT scientific_name FROM matched_sci)
      ...
      SELECT jsonb_build_object(
        ...
        'marcheur', (
          ...
          WHERE cp.user_id IN (SELECT u3.marcheur_id FROM unified_dedup u3
                                WHERE u3.scientific_name = sm.species_scientific_name
