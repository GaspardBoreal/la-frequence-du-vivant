...
function ActJardinMonde({ species, photos, testimonies }) {
  const p = useScrollProgress();
  const active = p > 0.78;
  ...
  return createElement('section', {
    ...
    createElement(motion.div, {
      ...
      Array.from({ length: Math.min(all, 200) }).map((_, i) => {
        ...
        const color = isPhoto ? '#fcd34d' : hashColor((item && item.iconic_taxon) || 'x');
    ...
    createElement('div', { className: 'absolute inset-x-0 bottom-12 text-center px-6' },
      ...
      }, species.length + ' espèces et ' + photos.length + ' regards'),
...
function Scenography({ data }) {
  ...
  return createElement('div', { className: 'relative min-h-[500vh]' },
    ...
    createElement(ActJardinMonde, { species, photos, testimonies }),
    createElement('div', {
      className: 'fixed top-3 left-3 z-50 text-[10px] uppercase tracking-[0.3em] text-emerald-100/40',
    }, 'la frequence du vivant'),
  );
}

export default Scenography;
$code$,
scenography_title = 'Un Jardin Monde — DEVIAT',
scenography_updated_at = now()
WHERE id = 'f6095e8d-44a8-4156-951f-dd604b821603';