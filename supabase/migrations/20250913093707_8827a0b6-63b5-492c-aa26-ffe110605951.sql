-- Fonction pour migrer automatiquement les données de vocabulaire mal catégorisées
CREATE OR REPLACE FUNCTION migrate_vocabulary_categorization()
RETURNS TABLE(opus_id uuid, changes_count integer, migration_log text) AS $$
DECLARE
    r record;
    old_data jsonb;
    new_data jsonb;
...
BEGIN
    ...
    FOR r IN 
        SELECT id, opus_id, vocabulaire_local 
