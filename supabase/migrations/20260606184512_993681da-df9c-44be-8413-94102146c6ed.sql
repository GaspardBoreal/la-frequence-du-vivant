    ...
    SELECT
      ...
      COALESCE(
        (SELECT st.common_name_fr FROM public.species_translations st
           WHERE st.scientific_name = o.species_scientific_name
    ...
    LEFT JOIN LATERAL (
      SELECT jsonb_build_object(
        ...
        'exploration_id', (
          ...
          WHERE em3.marche_id = o2.marche_id LIMIT 1
        ),
        'event_id', (
          ...
          WHERE me.exploration_id = (
            ...
            WHERE em4.marche_id = o2.marche_id LIMIT 1
          )
        ...
        'event_title', (
          ...
          WHERE me.exploration_id = (
            ...
            WHERE em5.marche_id = o2.marche_id LIMIT 1
          )
          ORDER BY me.date_marche DESC NULLS LAST
          LIMIT 1
        )
  ...
  SELECT * FROM (
    ...
    SELECT 'practice'::text, c.id::text,
      ...
      jsonb_build_object(
        ...
        'sense', c.sense,
        'thumb_url', (
          SELECT em.url FROM public.exploration_medias em
          WHERE em.id = ANY(c.media_ids)
          ORDER BY array_position(c.media_ids, em.id)
          LIMIT 1
        )
      )
